...
export class UOMService {
    ...
    get(param: Object): Observable<any> {
        return this.dataService.get(`${environment.api_root}/uom`, param)
            .pipe(map((data: any) => {
    ...
    post(param: Object): Observable<any> {
        return this.dataService.post(`${environment.api_root}/uom`, param)
            .pipe(map((data: any) => {
                // store user details and basic auth credentials in local storage to keep user logged in between page refreshes
                if (data.resultKey == 1) {
                    return data;
    ...
    update(param: Object): Observable<any> {
        return this.dataService.put(`${environment.api_root}/uom`, param)
            .pipe(map((data: any) => {
                // store user details and basic auth credentials in local storage to keep user logged in between page refreshes
                if (data.resultKey == 1) {
                    return data;
